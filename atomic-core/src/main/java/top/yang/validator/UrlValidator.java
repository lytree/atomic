...
public class UrlValidator {
    ...
    /**
     * Allow local URLs, such as http://localhost/ or http://machine/ . This enables a broad-brush check, for complex local machine name validation requirements you should create
     * your validator with a {@link RegexValidator} instead ({@link #UrlValidator(RegexValidator, long)})
     */
     ...
     *
     * @param schemes Pass in one or more url schemes to consider valid, passing in a null will default to "http,https,ftp" being valid. If a non-null schemes is specified then all
     *                valid schemes must be specified. Setting the ALLOW_ALL_SCHEMES option will ignore the contents of schemes.
     */
     ...
     *
     * @param options The options should be set using the public constants declared in this class.  To set multiple options you simply add them together.  For example,
     ...
     * Initialize a UrlValidator with the given validation options.
     *
     * @param authorityValidator Regular expression validator used to validate the authority part This allows the user to override the standard set of domains.
    ...
    /**
     * Validate scheme. If schemes[] was initialized to a non null, then only those schemes are allowed. Otherwise the default schemes are "http", "https", "ftp". Matching is
     * case-blind.
     *
    ...
    /**
     * Returns true if the authority is properly formatted.  An authority is the combination of hostname and port.  A <code>null</code> authority value is considered invalid. Note:
     * this implementation validates the domain unless a RegexValidator was provided. If a RegexValidator was supplied and it matches, then the authority is regarded as valid with
     * no further checks, otherwise the method checks against the AUTHORITY_PATTERN and the DomainValidator (ALLOW_LOCAL_URLS)
     *
    ...
    /**
     * Tests whether the given flag is on.  If the flag is not a power of 2 (e.g. 3) this tests whether the combination of flags is on.
     *
     * @param flag    Flag value to check.
