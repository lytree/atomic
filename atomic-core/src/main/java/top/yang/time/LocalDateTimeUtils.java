...
public class LocalDateTimeUtils {
    ...
    private final static String DEFAULT_FORMATTER = DatePattern.NORM_DATETIME_PATTERN;
     ...
     * 获取当前时间字符串
     *
     * @param dateFormatter 时间格式化格式
     * @return 格式化的时间
     */
    public static String getLocalDateTime(String dateFormatter) {
        LocalDateTime now = LocalDateTime.now();
    ...
    public static LocalDateTime parse(String text, String format) {
        ...
        if (StringUtils.isNotBlank(format)) {
            ...
            if (StringUtils.startsWithIgnoreCase(format, DatePattern.PURE_DATETIME_PATTERN)) {
                final String fraction = StringUtils.removeStart(format, DatePattern.PURE_DATETIME_PATTERN);
                if (RegExUtils.isMatch("[S]{1,2}", fraction)) {
                    //将yyyyMMddHHmmssS、yyyyMMddHHmmssSS的日期统一替换为yyyyMMddHHmmssSSS格式，用0补
                    text += StringUtils.repeat('0', 3 - fraction.length());
                }
