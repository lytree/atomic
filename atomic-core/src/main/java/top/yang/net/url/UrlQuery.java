package top.yang.net.url;

import java.nio.charset.Charset;
import java.util.IdentityHashMap;
import java.util.Iterator;
import java.util.Map;
import top.yang.codec.FormUrlencoded;
 ...
 * </pre>
 * 查询封装分为解析查询字符串和构建查询字符串，解析可通过charset为null来自定义是否decode编码后的内容，<br> 构建则通过charset是否为null是否encode参数键值对
 *
 * @author looly
 * @since 5.3.1
 */
public class UrlQuery {
    ...
    private final IdentityHashMap<String, String> query;
    /**
     ...
     * @return UrlQuery
     */
    public static UrlQuery of(Map<String, ?> queryMap) {
     ...
     * @return UrlQuery
     */
    public static UrlQuery of(Map<String, ?> queryMap, boolean isFormUrlEncoded) {
     ...
     * @param queryMap 初始化的查询键值对
     */
    public UrlQuery(Map<String, ?> queryMap) {
        this(queryMap, false);
    }
     ...
     * @since 5.7.16
     */
    public UrlQuery(Map<String, ?> queryMap, boolean isFormUrlEncoded) {
        if (MapUtils.isNotEmpty(queryMap)) {
            query = new IdentityHashMap<>(queryMap.size());
            addAll(queryMap);
        } else {
            query = new IdentityHashMap<>(16);
        }
     ...
     * @return this
     */
    public UrlQuery addAll(Map<String, ?> queryMap) {
        if (MapUtils.isNotEmpty(queryMap)) {
    ...
    public UrlQuery parse(String queryStr, Charset charset, boolean autoRemovePath) {
        ...
        if (autoRemovePath) {
            ...
            if (pathEndPos > -1) {
                queryStr = StringUtils.substringAfter(queryStr, pathEndPos + 1);
                if (StringUtils.isBlank(queryStr)) {
                    return this;
                }
            }
    ...
    /**
     * 构建URL查询字符串，即将key-value键值对转换为{@code key1=v1&key2=v2&key3=v3}形式。<br> 对于{@code null}处理规则如下：
     * <ul>
    ...
    /**
     * 生成查询字符串，类似于aaa=111&amp;bbb=222<br> 此方法不对任何特殊字符编码，仅用于输出显示
     *
     * @return 查询字符串
     */
    @Override
    ...
    /**
     * 解析URL中的查询字符串<br> 规则见：https://url.spec.whatwg.org/#urlencoded-parsing
     *
     * @param queryStr 查询字符串，类似于key1=v1&amp;key2=&amp;key3=v3
     * @param charset  decode编码，null表示不做decode
     * @return this
