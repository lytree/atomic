...
/**
 * 十六进制（简写为hex或下标16）在数学中是一种逢16进1的进位制，一般用数字0到9和字母A到F表示（其中:A~F即10~15）。<br> 例如十进制数57，在二进制写作111001，在16进制写作39。<br> 像java,c这样的语言为了区分十六进制和十进制数值,会在十六进制数的前面加上
...
public class HexUtil {
    ...
    public static String encodeHexString(String data, Charset charset) {
        return encodeHexString(StringUtils.getBytes(data, charset), true);
    }
    ...
    public static String decodeHexString(String hexStr) {
        return decodeHexString(hexStr, StandardCharsets.UTF_8);
    }
     ...
     * @return 字符串
     */
    public static String decodeHexString(String hexStr, Charset charset) {
        if (StringUtils.isEmpty(hexStr)) {
            return hexStr;
        }
        return decodeHexString(hexStr.toCharArray(), charset);
    }
