...
/**
 * Abstracts an OS' file system details, currently supporting the single use case of converting a file name String to a legal file name with {@link #toLegalFileName(String,
 * char)}.
 * <p>
...
public enum FileSystem {
     ...
     * <p>
     * If a character with value {@code searchChar} occurs in the character sequence represented by the {@code cs} object at an index no smaller than {@code start}, then the index
     * of the first such occurrence is returned. For values of {@code searchChar} in the range from 0 to 0xFFFF (inclusive), this is the smallest value <i>k</i> such that:
     * </p>
     * <blockquote><pre>
     ...
     * </p>
     * <p>
     * There is no restriction on the value of {@code start}. If it is negative, it has the same effect as if it were zero: the entire {@code CharSequence} may be searched. If it
     * is greater than the length of {@code cs}, it has the same effect as if it were equal to the length of {@code cs}: {@code -1} is returned.
     * </p>
    ...
    /**
     * Tests if a candidate file name (without a path) such as {@code "filename.ext"} or {@code "filename"} is a potentially legal file name. If the file name length exceeds {@link
    ...
    /**
     * Converts a candidate file name (without a path) like {@code "filename.ext"} or {@code "filename"} to a legal file name. Illegal characters in the candidate name are replaced
     * by the {@code replacement} character. If the file name length exceeds {@link #getMaxFileNameLength()}, then the name is truncated to {@link #getMaxFileNameLength()}.
     *
     * @param candidate   a candidate file name (without a path) like {@code "filename.ext"} or {@code "filename"}
