 ...
 */
package top.yang.io.input;

import static top.yang.io.IOUtils.EOF;
...
import top.yang.io.output.WriterOutputStream;
import top.yang.lang.CharsetEncoders;
 ...
 * transformed using a {@link CharsetEncoder} object, guaranteeing that all charset encodings supported by the JRE are handled correctly. In particular for charsets such as UTF-16,
 * the implementation ensures that one and only one byte order marker is produced.
 * <p>
 * Since in general it is not possible to predict the number of characters to be read from the {@link Reader} to satisfy a read request on the {@link ReaderInputStream}, all reads
 * from the {@link Reader} are buffered. There is therefore no well defined correlation between the current position of the {@link Reader} and that of the {@link
 * ReaderInputStream}. This also implies that in general there is no need to wrap the underlying {@link Reader} in a {@link java.io.BufferedReader}.
 * </p>
 * <p>
 * {@link ReaderInputStream} implements the inverse transformation of {@link java.io.InputStreamReader}; in the following example, reading from {@code in2} would return the same
 * byte sequence as reading from {@code in} (provided that the initial byte sequence is legal with respect to the charset encoding):
 * </p>
 *
 * <pre>
 ...
 * stream into a byte stream, but {@link java.io.OutputStreamWriter} pushes data to the underlying stream, while {@link ReaderInputStream} pulls it from the underlying stream.
 * </p>
 * <p>
 ...
 * code. This class is typically used in situations where an existing API only accepts an {@link InputStream}, but where the most natural way to produce the data is as a character
 * stream, i.e. by providing a {@link Reader} instance. An example of a situation where this problem may appear is when implementing the {@code javax.activation.DataSource}
...
public class ReaderInputStream extends InputStream {
     ...
     *
     * <p>
     * This constructor does not call {@link CharsetEncoder#reset() reset} on the provided encoder. The caller of this constructor should do this when providing an encoder which
