...
/**
 * An {@link InputStream} that utilizes memory mapped files to improve performance. A sliding window of the file is mapped to memory to avoid mapping the entire file to memory at
 * one time. The size of the sliding buffer is configurable.
 * <p>
 * For most operating systems, mapping a file into memory is more expensive than reading or writing a few tens of kilobytes of data. From the standpoint of performance. it is
 * generally only worth mapping relatively large files into memory.
 * </p>
 * <p>
 * Note: Use of this class does not necessarily obviate the need to use a {@link BufferedInputStream}. Depending on the use case, the use of buffering may still further improve
...
public class MemoryMappedFileInputStream extends InputStream {
    ...
    /**
     * Default size of the sliding memory mapped buffer. We use 256K, equal to 65536 pages (given a 4K page size). Increasing the value beyond the default size will generally not
     * provide any increase in throughput.
     */
