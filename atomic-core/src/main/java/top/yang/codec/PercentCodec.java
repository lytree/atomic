...
public class PercentCodec implements Serializable {
    ...
    /**
     * 是否编码空格为+<br> 如果为{@code true}，则将空格编码为"+"，此项只在"application/x-www-form-urlencoded"中使用<br> 如果为{@code false}，则空格编码为"%20",此项一般用于URL的Query部分（RFC3986规范）
     */
    private boolean encodeSpaceAsPlus = false;

    /**
     * 构造<br> [a-zA-Z0-9]默认不被编码
     */
    public PercentCodec() {
        this(new BitSet(256));
    }
    ...
    /**
     * 是否将空格编码为+<br> 如果为{@code true}，则将空格编码为"+"，此项只在"application/x-www-form-urlencoded"中使用<br> 如果为{@code false}，则空格编码为"%20",此项一般用于URL的Query部分（RFC3986规范）
     *
