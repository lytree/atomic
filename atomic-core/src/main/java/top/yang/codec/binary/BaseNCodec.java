 ...
 * </p>
 * <p>
 * You can set the decoding behavior when the input bytes contain leftover trailing bits that cannot be created by a valid encoding. These can be bits that are unused from the
 * final character or entire characters. The default mode is lenient decoding.
 * <ul>
...
public abstract class BaseNCodec implements BinaryEncoder, BinaryDecoder {
    ...
    static class Context {
        ...
        /**
         * Variable tracks how many characters have been written to the current line. Only used when encoding. We use it to make sure each encoded line never goes beyond lineLength
         * (if lineLength &gt; 0).
         */
        int currentLinePos;
     ...
     * @return the value {@code 0} if {@code x == y}; a value less than {@code 0} if {@code x < y} as unsigned values; and a value greater than {@code 0} if {@code x > y} as
     * unsigned values
     */
    ...
    /**
     * Create a positive capacity at least as large the minimum required capacity. If the minimum capacity is negative then this throws an OutOfMemoryError as no array can be
     * allocated.
     *
    ...
    /**
     * Defines the decoding behavior when the input bytes contain leftover trailing bits that cannot be created by a valid encoding. These can be bits that are unused from the
     * final character or entire characters. The default mode is lenient decoding. Set this to {@code true} to enable strict decoding.
     * <ul>
    ...
    /**
     * Encodes an Object using the Base-N algorithm. This method is provided in order to satisfy the requirements of the Encoder interface, and will throw an EncoderException if
     * the supplied object is not of type byte[].
     *
     ...
     *
     * <p>
     * The default is lenient. If the decoding policy is strict, then decoding will raise an {@link IllegalArgumentException} if trailing bits are not part of a valid encoding.
     * Decoding will compose trailing bits into 8-bit bytes and discard the remainder.
     * </p>
     *
