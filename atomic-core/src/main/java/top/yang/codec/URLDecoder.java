 ...
 * @author looly
 */
public class URLDecoder extends java.net.URLDecoder implements Serializable {
    ...
    /**
     * 解码<br> 规则见：https://url.spec.whatwg.org/#urlencoded-parsing
     * <pre>
     *   1. 将+和%20转换为空格(" ");
     *   2. 将"%xy"转换为文本形式,xy是两位16进制的数值;
     *   3. 跳过不符合规范的%形式，直接输出
    ...
    public static String decode(String str, Charset charset, boolean isPlusToSpace) {
        ...
        return StringUtils.toString(decode(StringUtils.getBytes(str, charset), isPlusToSpace), charset);
    }
    ...
    public static byte[] decode(byte[] bytes, boolean isPlusToSpace) {
        ...
        for (int i = 0; i < bytes.length; i++) {
            b = bytes[i];
            if (b == '+') {
                buffer.write(isPlusToSpace ? CharUtils.SPACE : b);
