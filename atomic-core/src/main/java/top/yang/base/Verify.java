 ...
 * except that they are always enabled. These methods should be used instead of Java assertions whenever there is a chance the check may fail "in real life". Example:
 *
 * <pre>{@code
 ...
 * which approach to use, <b>don't worry</b> too much about it; just pick something that seems reasonable and it will be fine.
 *
 * <ul>
 ...
 *
 * <p>As with {@link Preconditions}, {@code Verify} uses {@link StringUtils.format} to format
...
public final class Verify {
    ...
    /**
     * Ensures that {@code expression} is {@code true}, throwing a {@code VerifyException} with a custom message otherwise.
     *
    ...
    public static void verify(
        ...
        if (!expression) {
            throw new VerifyException(StringUtils.format(errorMessageTemplate, errorMessageArgs));
        }
    }
    ...
    public static void verify(boolean expression, String errorMessageTemplate, char p1) {
        if (!expression) {
            throw new VerifyException(StringUtils.format(errorMessageTemplate, p1));
        }
    }
    ...
    public static void verify(boolean expression, String errorMessageTemplate, char p1, char p2) {
        if (!expression) {
            throw new VerifyException(StringUtils.format(errorMessageTemplate, p1, p2));
        }
    }
    ...
    public static void verify(
            ...
            Object p3) {
        if (!expression) {
            throw new VerifyException(StringUtils.format(errorMessageTemplate, p1, p2, p3));
        }
    }
    ...
    public static void verify(
            ...
            Object p4) {
        if (!expression) {
            throw new VerifyException(StringUtils.format(errorMessageTemplate, p1, p2, p3, p4));
        }
    }
