...
import static top.lytree.collections.Hashing.smearedHash;
...
public final class HashBiMap<K, V>
    ...
    /**
     * Constructs a new bimap containing initial values from {@code map}. The bimap is created with an initial capacity sufficient to hold the mappings in the specified map.
     */
    public static <K, V> HashBiMap<K, V> create(
...
//       todo checkNonnegative(expectedSize, "expectedSize");
    ...
    /**
     * Returns {@code true} if this BiMap contains an entry whose value is equal to {@code value} (or, equivalently, if this inverse view contains a key that is equal to {@code
     * value}).
     *
