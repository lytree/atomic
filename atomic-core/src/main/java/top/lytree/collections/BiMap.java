...
/**
 * A bimap (or "bidirectional map") is a map that preserves the uniqueness of its values as well as that of its keys. This constraint enables bimaps to support an "inverse view",
...
public interface BiMap<K extends Object, V extends Object> extends Map<K, V> {
     ...
     * {@inheritDoc}
     *
     * @throws IllegalArgumentException if the given value is already bound to a different key in this bimap. The bimap will remain unmodified in this event. To avoid this
     ...
     * @return the value that was previously associated with the key, or {@code null} if there was no previous entry. (If the bimap contains null values, then {@code forcePut},
     * like {@code put}, returns {@code null} both if the key is absent and if it is present with a null value.)
     */
    ...
    /**
     * Returns the inverse view of this bimap, which maps each of this bimap's values to its associated key. The two bimaps are backed by the same data; any changes to one will
     * appear in the other.
     *
