...
/**
 * A Proxy stream which acts as expected, that is it passes the method calls on to the proxied stream and doesn't change which methods are being called. It is an alternative base
 * class to FilterWriter to increase reusability, because FilterWriter changes the methods being called, such as {@code write(char[]) to write(char[], int, int)} and {@code
 * write(String) to write(String, int, int)}.
 */
public class ProxyWriter extends FilterWriter {
    ...
    /**
     * Invoked by the write methods after the proxied call has returned successfully. The number of chars written (1 for the {@link #write(int)} method, buffer length for {@link
     * #write(char[])}, etc.) is given as an argument.
     * <p>
    ...
    /**
     * Invoked by the write methods before the call is proxied. The number of chars to be written (1 for the {@link #write(int)} method, buffer length for {@link #write(char[])},
