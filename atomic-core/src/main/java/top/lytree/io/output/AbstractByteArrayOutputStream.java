...
import top.lytree.io.input.ClosedInputStream;
 ...
 * <p>
 * The data can be retrieved using {@code toByteArray()} and {@code toString()}. Closing an {@code AbstractByteArrayOutputStream} has no effect. The methods in this class can be
 * called after the stream has been closed without generating an {@code IOException}.
 * </p>
 * <p>
 * This is the base for an alternative implementation of the {@link java.io.ByteArrayOutputStream} class. The original implementation only allocates 32 bytes at the beginning. As
 * this class is designed for heavy duty it starts at {@value #DEFAULT_SIZE} bytes. In contrast to the original it doesn't reallocate the whole memory block but allocates
 * additional buffers. This way no buffers need to be garbage collected and the contents don't have to be copied to the new buffer. This class is designed to behave exactly like
...
public abstract class AbstractByteArrayOutputStream extends OutputStream {
    ...
    /**
     * Gets the current contents of this byte stream as an Input Stream. The returned stream is backed by buffers of {@code this} stream, avoiding memory allocation and copy, thus
     ...
     * @param isConstructor A constructor for an InputStream which makes up the {@link SequenceInputStream}.
     * @return the current contents of this output stream.
     * @see java.io.ByteArrayOutputStream#toByteArray()
     * @see #reset()
     * 
