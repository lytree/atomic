package top.lytree.time.chinese;
...
public class NumberChineseFormatter {
    ...
    public static String format(double amount, final boolean isUseTraditional, final boolean isMoneyMode, final String negativeName, final String unitName) {
        ...
        // 负数
        if (amount < 0) {
            chineseStr.append(StringUtils.isNullOrUndefined(negativeName) ? "负" : negativeName);
            amount = -amount;
        }
        ...
        if (false == isMoneyMode || 0 != yuan) {
            ...
            if (isMoneyMode) {
                chineseStr.append(StringUtils.isNullOrUndefined(unitName) ? "元" : unitName);
            }
        }
    ...
    public static String formatSimple(final long amount) {
        ...
        if (amount < 1_0000_0000 && amount > -1_0000_0000) {
            res = MathUtils.div(amount, 1_0000, 2) + "万";
        } else if (amount < 1_0000_0000_0000L && amount > -1_0000_0000_0000L) {
            res = MathUtils.div(amount, 1_0000_0000, 2) + "亿";
        } else {
            res = MathUtils.div(amount, 1_0000_0000_0000L, 2) + "万亿";
        }
        return res;
    }
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (num >= 0) {
                if (num == 0) {
                    ...
                    // 多个数字同时出现，报错
                    throw new IllegalArgumentException(StringUtils.format("Bad number '{}{}' at: {}", chinese.charAt(i - 1), c, i));
                }
                // 普通数字
                number = num;
            } else {
                ...
                if (null == unit) {
                    // 出现非法字符
                    throw new IllegalArgumentException(StringUtils.format("Unknown unit '{}' at: {}", c, i));
                }
    ...
    public static BigDecimal chineseMoneyToNumber(final String chineseMoneyAmount) {
        if (StringUtils.isBlank(chineseMoneyAmount)) {
            return null;
        }
    ...
    private static int chineseToNumber(char chinese) {
        ...
        final int i = ArrayUtils.indexOf(DIGITS, chinese);
        if (i > 0) {
            return (i + 1) / 2;
        }
        return i;
