...
/**
 * Provides Base32 encoding and decoding in a streaming fashion (unlimited size). When encoding the default lineLength is 76 characters and the default lineEnding is CRLF, but
 * these can be overridden by using the appropriate constructor.
 * <p>
 * The default behavior of the Base32OutputStream is to ENCODE, whereas the default behavior of the Base32InputStream is to DECODE. But this behavior can be overridden by using a
...
public class Base32OutputStream extends BaseNCodecOutputStream {
     ...
     * @param doEncode      true if we should encode all data written to us, false if we should decode.
     * @param lineLength    If doEncode is true, each line of encoded data will contain lineLength characters (rounded down to nearest multiple of 4). If lineLength &lt;= 0, the
