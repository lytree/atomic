...
import top.lytree.codec.CodecPolicy;
...
public class Base32 extends BaseNCodec {
    ...
    /**
     * This array is a lookup table that translates Unicode characters drawn from the "Base32 Alphabet" (as specified in Table 3 of RFC 4648) into their 5-bit positive integer
     * equivalents. Characters that are not in the Base32 alphabet but fall within the bounds of the array are translated to -1.
     */
    ...
    /**
     * This array is a lookup table that translates Unicode characters drawn from the "Base32 Hex Alphabet" (as specified in Table 4 of RFC 4648) into their 5-bit positive integer
     * equivalents. Characters that are not in the Base32 Hex alphabet but fall within the bounds of the array are translated to -1.
     */
    ...
    /**
     * Convenience variable to help us determine when our buffer is going to run out of room and needs resizing. {@code decodeSize = {@link #BYTES_PER_ENCODED_BLOCK} - 1 +
    ...
    /**
     * Convenience variable to help us determine when our buffer is going to run out of room and needs resizing. {@code encodeSize = {@link #BYTES_PER_ENCODED_BLOCK} +
     * lineSeparator.length;}
     */
     ...
     * </p>
     *
     * @param lineLength Each line of encoded data will be at most of the given length (rounded down to nearest multiple of 8). If lineLength &lt;= 0, then the output will not be
     ...
     * </p>
     *
     * @param lineLength    Each line of encoded data will be at most of the given length (rounded down to nearest multiple of 8). If lineLength &lt;= 0, then the output will not
     ...
     * </p>
     * <p>
     * Ignores all non-Base32 characters. This is how chunked (e.g. 76 character) data is handled, since CR and LF are silently ignored, but has implications for other bytes, too.
     * This method subscribes to the garbage-in, garbage-out philosophy: it will not check the provided data for validity.
     * </p>
     * <p>
     * Output is written to {@link BaseNCodec.Context#buffer Context#buffer} as 8-bit octets, using {@link
     * BaseNCodec.Context#pos Context#pos} as the buffer position
     * </p>
     *
...
//              case 0 : // impossible, as excluded above
    ...
    /**
     * <p>
     * Encodes all of the provided data, starting at inPos, for inAvail bytes. Must be called at least twice: once with the data to encode, and once with inAvail set to "-1" to
     * alert encoder that EOF has been reached, so flush last remaining bytes (if not multiple of 5).
     * </p>
     *
