...
public abstract class AbstractFtp implements Closeable {
    ...
    public boolean exist(final String path) {
        ...
        if (CharUtils.isFileSeparator(path.charAt(path.length() - 1))) {
            return false;
        }
        ...
        // 文件验证
        final String dir = StringUtils.defaultIfEmpty(StringUtils.removeEnd(path, fileName), ".");
    ...
    public void download(final String path, File outFile, String tempFileSuffix) {
        if (StringUtils.isBlank(tempFileSuffix)) {
            tempFileSuffix = ".temp";
        } else {
            tempFileSuffix = StringUtils.appendIfMissing(tempFileSuffix, StringUtils.DOT);
        }

        // 目标文件真实名称
        final String fileName = outFile.isDirectory() ? FilenameUtils.getName(path) : outFile.getName();
        // 临时文件名称
        ...
        try {
            ...
            // 重命名下载好的临时文件
            FileUtils.copyFile(outFile, new File(fileName), true);
