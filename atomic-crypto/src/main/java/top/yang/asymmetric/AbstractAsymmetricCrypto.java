...
public abstract class AbstractAsymmetricCrypto<T extends AbstractAsymmetricCrypto<T>> extends BaseAsymmetric<T> {
   ...
   * 构造
   * <p>
   * 私钥和公钥同时为空时生成一对新的私钥和公钥<br> 私钥和公钥可以单独传入一个，如此则只能使用此钥匙来做加密或者解密
   *
   * @param algorithm  算法
   * @param privateKey 私钥
   * @param publicKey  公钥
  ...
  public byte[] encrypt(String data, String charset, KeyType keyType) {
    return encrypt(StringUtils.toBytes(data, charset), keyType);
  }
   ...
   */
  public byte[] encrypt(InputStream data, KeyType keyType) throws IOException {
    return encrypt(IOUtils.buffer(data), keyType);
  }
   ...
   * @throws IOException IO异常
   */
  public byte[] decrypt(InputStream data, KeyType keyType) throws IOException {
    return decrypt(IOUtils.buffer(data), keyType);
  }
