...
import top.yang.BCUtil;
import top.yang.CryptoException;
import top.yang.ECKeyUtil;
import top.yang.SecureUtil;
import top.yang.math.HexUtil;

/**
 * 国密SM2算法实现，基于BC库<br> SM2算法只支持公钥加密，私钥解密<br> 参考：https://blog.csdn.net/pridas/article/details/86118774
 *
 * @author looly
 * @since 4.3.2
 */
public class SM2 extends AbstractAsymmetricCrypto<SM2> {
  ...
  /**
   * 构造<br> 私钥和公钥同时为空时生成一对新的私钥和公钥<br> 私钥和公钥可以单独传入一个，如此则只能使用此钥匙来做加密或者解密
   *
  ...
  private SM2Engine getEngine() {
    if (null == this.engine) {
      if (Objects.isNull(this.digest)) {
        throw new IllegalArgumentException("digest must be not null !");
      }
