...
import top.yang.asymmetric.AsymmetricAlgorithm;
import top.yang.asymmetric.RSA;
import top.yang.asymmetric.Sign;
import top.yang.asymmetric.SignAlgorithm;
import top.yang.codec.Base64;
import top.yang.digest.DigestAlgorithm;
import top.yang.digest.Digester;
import top.yang.digest.HMac;
import top.yang.digest.HmacAlgorithm;
import top.yang.digest.MD5;
import top.yang.math.HexUtil;
import top.yang.regex.RegExUtils;
import top.yang.string.StringUtils;
import top.yang.symmetric.AES;
import top.yang.symmetric.DES;
import top.yang.symmetric.DESede;
import top.yang.symmetric.PBKDF2;
import top.yang.symmetric.RC4;
import top.yang.symmetric.SymmetricCrypto;
...
public class SecureUtil {
   ...
   * @since 4.4.1
   */
  public static String generateAlgorithm(AsymmetricAlgorithm asymmetricAlgorithm,
      DigestAlgorithm digestAlgorithm) {
    final String digestPart = (null == digestAlgorithm) ? "NONE" : digestAlgorithm.name();
    return digestPart + "with" + asymmetricAlgorithm.getValue();
  }
  ...
  public static HMac hmac(HmacAlgorithm algorithm, String key) {
    return new HMac(algorithm, StringUtils.toBytes(key));
  }
  ...
  public static HMac hmacMd5(String key) {
    return hmacMd5(StringUtils.toString(key));
  }
  ...
  public static HMac hmacSha1(String key) {
    return hmacSha1(StringUtils.toString(key));
  }
  ...
  public static HMac hmacSha256(String key) {
    return hmacSha256(StringUtils.toString(key));
  }
...
//	}
  ...
  public static byte[] decode(String key) {
    return RegExUtils.isHex(key) ? HexUtil.decodeHex(key) : Base64.decode(key);
  }
   ...
   * @return {@link RC4}
   */
  public static RC4 rc4(String key) throws UnsupportedEncodingException {
    return new RC4(key);
  }
