...
import top.yang.asymmetric.SM2;
import top.yang.collections.ArrayUtils;
import top.yang.digest.HMac;
import top.yang.digest.HmacAlgorithm;
import top.yang.digest.SM3;
import top.yang.digest.mac.BCHMacEngine;
import top.yang.digest.mac.MacEngine;
import top.yang.symmetric.SM4;
import top.yang.symmetric.SymmetricCrypto;

/**
 * SM国密算法工具类<br> 此工具类依赖org.bouncycastle:bcprov-jdk15to18
 *
 * <p>封装包括：</p>
 * <ul>
...
public class SmUtil {
  ...
  /**
   * 创建SM2算法对象<br> 私钥和公钥同时为空时生成一对新的私钥和公钥<br> 私钥和公钥可以单独传入一个，如此则只能使用此钥匙来做加密或者解密
   *
  ...
  /**
   * SM3加密<br> 例：<br> SM3加密：sm3().digest(data)<br> SM3加密并转为16进制字符串：sm3().digestHex(data)<br>
   *
   * @return {@link SM3}
   */
  ...
  /**
   * SM4加密，生成随机KEY。注意解密时必须使用相同 {@link SymmetricCrypto}对象或者使用相同KEY<br> 例：
   *
   * <pre>
