...
public class KeyUtil {
    ...
    /**
     * 生成 {@link SecretKey}，仅用于对称加密和摘要算法密钥生成<br> 当指定keySize&lt;0时，AES默认长度为128，其它算法不指定。
     *
     * @param algorithm 算法，支持PBE算法
    ...
    public static SecretKey generateKey(String algorithm, byte[] key) {
        ...
        if (algorithm.startsWith("PBE")) {
            // PBE密钥
            secretKey = generatePBEKey(algorithm, (null == key) ? null : StringUtils.toString(key).toCharArray());
    ...
    /**
     * 生成RSA私钥，仅用于非对称加密<br> 采用PKCS#8规范，此规范定义了私钥信息语法和加密私钥语法<br> 算法见：https://docs.oracle.com/javase/7/docs/technotes/guides/security/StandardNames.html#KeyFactory
     *
    ...
    /**
     * 生成私钥，仅用于非对称加密<br> 采用PKCS#8规范，此规范定义了私钥信息语法和加密私钥语法<br> 算法见：https://docs.oracle.com/javase/7/docs/technotes/guides/security/StandardNames.html#KeyFactory
     *
    ...
    /**
     * 生成RSA公钥，仅用于非对称加密<br> 采用X509证书规范<br> 算法见：https://docs.oracle.com/javase/7/docs/technotes/guides/security/StandardNames.html#KeyFactory
     *
     * @param key 密钥，必须为DER编码存储
    ...
    public static String getMainAlgorithm(String algorithm) {
        ...
        final int slashIndex = algorithm.indexOf(StringUtils.SLASH);
        if (slashIndex > 0) {
    ...
    /**
     * 读取PKCS12 KeyStore文件<br> KeyStore文件用于数字证书的密钥对保存
     *
     * @param keyFile  证书文件
     * @param password 密码
     * @return {@link KeyStore}
    ...
    public static KeyStore readKeyStore(String type, File keyFile, char[] password) throws IOException {
        InputStream in = null;
        try {
            in = FileUtils.openInputStream(keyFile);
    ...
    /**
     * 读取X.509 Certification文件中的公钥<br> Certification为证书文件<br> see: https://www.cnblogs.com/yinliang/p/10115519.html
     *
     * @param in {@link InputStream} 如果想从文件读取.cer文件，使用 {@link FileUtils#openInputStream(File)} 读取
     * @return {@link KeyStore}
     *
     */
    ...
    /**
     * 编码压缩EC公钥（基于BouncyCastle）<br> 见：https://www.cnblogs.com/xinzhao/p/8963724.html
     *
