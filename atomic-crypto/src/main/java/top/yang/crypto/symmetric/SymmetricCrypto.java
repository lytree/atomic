...
import top.yang.crypto.CipherMode;
...
public class SymmetricCrypto implements SymmetricEncryptor, SymmetricDecryptor, Serializable {
    ...
    /**
     * 更新数据，分组加密中间结果可以当作随机数<br> 第一次更新数据前需要调用{@link #setMode(CipherMode)}初始化加密或解密模式，然后每次更新数据都是累加模式
     *
    ...
    @Override
    public void encrypt(InputStream data, OutputStream out, boolean isClose) throws IOException, InvalidAlgorithmParameterException, InvalidKeyException {
    ...
    @Override
    public void decrypt(InputStream data, OutputStream out, boolean isClose) throws IOException, InvalidAlgorithmParameterException, InvalidKeyException {
        ...
        try {
            ...
            IOUtils.copyLarge(cipherInputStream, out);
        } finally {
            lock.unlock();
            // issue#I4EMST@Gitee
            // CipherOutputStream必须关闭，才能完全写出
