...
import top.yang.crypto.asymmetric.Sign;
import top.yang.crypto.asymmetric.SignAlgorithm;
import top.yang.crypto.digest.DigestAlgorithm;
import top.yang.crypto.digest.Digester;
import top.yang.crypto.digest.HMac;
import top.yang.crypto.digest.HmacAlgorithm;
import top.yang.crypto.digest.MD5;
import top.yang.crypto.symmetric.AES;
import top.yang.crypto.symmetric.DES;
import top.yang.crypto.symmetric.DESede;
import top.yang.crypto.symmetric.PBKDF2;
import top.yang.crypto.symmetric.RC4;
import top.yang.crypto.symmetric.SymmetricCrypto;
import top.yang.crypto.symmetric.ZUC;
import top.yang.crypto.symmetric.fpe.FPE;
...
public class SecureUtil {
    ...
    /**
     * 生成公钥，仅用于非对称加密<br> 算法见：https://docs.oracle.com/javase/7/docs/technotes/guides/security/StandardNames.html#KeyFactory
     *
     * @param algorithm 算法
     * @param key       密钥
    ...
    /**
     * 生成用于非对称加密的公钥和私钥，仅用于非对称加密<br> 密钥对生成算法见：https://docs.oracle.com/javase/7/docs/technotes/guides/security/StandardNames.html#KeyPairGenerator
     *
    ...
    /**
     * 获取用于密钥生成的算法<br> 获取XXXwithXXX算法的后半部分算法，如果为ECDSA或SM2，返回算法为EC
     *
    ...
    /**
     * 读取密钥库(Java Key Store，JKS) KeyStore文件<br> KeyStore文件用于数字证书的密钥对保存<br> see: http://snowolf.iteye.com/blog/391931
     *
     * @param in       {@link InputStream} 如果想从文件读取.keystore文件，使用 {@link FileUtils#openInputStream(java.io.File)} 读取
     * @param password 密码
     ...
     *
     * @param type 类型，例如X.509
     * @param in   {@link InputStream} 如果想从文件读取.cer文件，使用 {@link FileUtils#openInputStream(File)}  (java.io.File)} 读取
    ...
    /**
     * AES加密，生成随机KEY。注意解密时必须使用相同 {@link AES}对象或者使用相同KEY<br> 例：
     *
     * <pre>
    ...
    /**
     * DES加密，生成随机KEY。注意解密时必须使用相同 {@link DES}对象或者使用相同KEY<br> 例：
     *
     * <pre>
    ...
    /**
     * DESede加密（又名3DES、TripleDES），生成随机KEY。注意解密时必须使用相同 {@link DESede}对象或者使用相同KEY<br> Java中默认实现为：DESede/ECB/PKCS5Padding<br> 例：
     *
     * <pre>
    ...
    /**
     * DESede加密（又名3DES、TripleDES）<br> Java中默认实现为：DESede/ECB/PKCS5Padding<br> 例：
     *
     * <pre>
    ...
    /**
     * SHA1加密<br> 例：<br> SHA1加密：sha1().digest(data)<br> SHA1加密并转为16进制字符串：sha1().digestHex(data)<br>
     *
     * @return {@link Digester}
     */
    ...
    /**
     * HmacMD5加密器<br> 例：<br> HmacMD5加密：hmacMd5(key).digest(data)<br> HmacMD5加密并转为16进制字符串：hmacMd5(key).digestHex(data)<br>
     *
    ...
    /**
     * HmacMD5加密器，生成随机KEY<br> 例：<br> HmacMD5加密：hmacMd5().digest(data)<br> HmacMD5加密并转为16进制字符串：hmacMd5().digestHex(data)<br>
     *
     * @return {@link HMac}
     */
    ...
    /**
     * HmacSHA1加密器<br> 例：<br> HmacSHA1加密：hmacSha1(key).digest(data)<br> HmacSHA1加密并转为16进制字符串：hmacSha1(key).digestHex(data)<br>
     *
     * @param key 加密密钥，如果为{@code null}生成随机密钥
     * @return {@link HMac}
     * @since 3.3.0
    ...
    /**
     * HmacSHA1加密器，生成随机KEY<br> 例：<br> HmacSHA1加密：hmacSha1().digest(data)<br> HmacSHA1加密并转为16进制字符串：hmacSha1().digestHex(data)<br>
     *
     * @return {@link HMac}
     */
    ...
    /**
     * HmacSHA256加密器<br> 例：<br> HmacSHA256加密：hmacSha256(key).digest(data)<br> HmacSHA256加密并转为16进制字符串：hmacSha256(key).digestHex(data)<br>
     *
     * @param key 加密密钥，如果为{@code null}生成随机密钥
     * @return {@link HMac}
     * @since 5.6.0
    ...
    /**
     * 创建RSA算法对象<br> 私钥和公钥同时为空时生成一对新的私钥和公钥<br> 私钥和公钥可以单独传入一个，如此则只能使用此钥匙来做加密或者解密
     *
    ...
    /**
     * 创建Sign算法对象<br> 私钥和公钥同时为空时生成一对新的私钥和公钥<br> 私钥和公钥可以单独传入一个，如此则只能使用此钥匙来做签名或验证
     *
    ...
    /**
     * 对参数做md5签名<br> 参数签名为对Map参数按照key的顺序排序后拼接为字符串，然后根据提供的签名算法生成签名字符串<br> 拼接后的字符串键值对之间无符号，键值对之间无符号，忽略null值
     *
     * @param params      参数
     * @param otherParams 其它附加参数字符串（例如密钥）
     * @return 签名
    ...
    /**
     * 对参数做Sha1签名<br> 参数签名为对Map参数按照key的顺序排序后拼接为字符串，然后根据提供的签名算法生成签名字符串<br> 拼接后的字符串键值对之间无符号，键值对之间无符号，忽略null值
     *
     * @param params      参数
     * @param otherParams 其它附加参数字符串（例如密钥）
     * @return 签名
    ...
    /**
     * 对参数做Sha256签名<br> 参数签名为对Map参数按照key的顺序排序后拼接为字符串，然后根据提供的签名算法生成签名字符串<br> 拼接后的字符串键值对之间无符号，键值对之间无符号，忽略null值
     *
     * @param params      参数
     * @param otherParams 其它附加参数字符串（例如密钥）
     * @return 签名
