...
public class Digester implements Serializable {
  ...
  /**
   * 设置加盐的位置，只有盐值存在时有效<br> 加盐的位置指盐位于数据byte数组中的位置，例如：
   *
   * <pre>
   * data: 0123456
   * </pre>
  ...
  public byte[] digest(String data, String charsetName) {
    return digest(data, CharsetsUtils.toCharsetName(charsetName));
  }
  ...
  public String digestHex(String data, String charsetName) {
    return digestHex(data, CharsetsUtils.toCharsetName(charsetName));
  }
...
//  /**
//   * 生成文件摘要<br> 使用默认缓存大小，见 {@link IoUtil#DEFAULT_BUFFER_SIZE}
//   *
//   * @param file 被摘要文件
//   * @return 摘要bytes
//   * @throws CryptoException Cause by IOException
...
//  /**
//   * 生成文件摘要，并转为16进制字符串<br> 使用默认缓存大小，见 {@link IoUtil#DEFAULT_BUFFER_SIZE}
//   *
//   * @param file 被摘要文件
//   * @return 摘要
//   */
...
//  }
  ...
  /**
   * 生成摘要，并转为16进制字符串<br> 使用默认缓存大小，见 {@link IOUtils#DEFAULT_BUFFER_SIZE}
   *
   * @param data 被摘要数据
   * @return 摘要
   */
  ...
  /**
   * 重复计算摘要，取决于{@link #digestCount} 值<br> 每次计算摘要前都会重置{@link #digest}
   *
