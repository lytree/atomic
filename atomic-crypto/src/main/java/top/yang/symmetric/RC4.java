...
/**
 * RC4加密解密算法实现<br> 来自：https://github.com/xSAVIKx/RC4-cipher/blob/master/src/main/java/com/github/xsavikx/rc4/RC4.java
 *
...
public class RC4 implements Serializable {
  ...
  public byte[] encrypt(String message, Charset charset) throws CryptoException {
    return crypt(StringUtils.getBytes(message, charset));
  }
  ...
  public void setKey(String key) throws CryptoException, UnsupportedEncodingException {
    ...
    writeLock.lock();
    try {
      this.sbox = initSBox(StringUtils.getBytes(key));
    } finally {
      writeLock.unlock();
    }
  }
